...
// components
import CardEditMusicStyle from 'components/BackOffice/Cards/CardEditMusicStyle';
...
export async function getServerSideProps(context) {
  ...
  const response = await prisma.mUSIC_STYLE.findFirst({
    where: { ID: parseInt(context.params.id) },
  });
  await prisma.$disconnect();
  const result = await JSON.parse(JSON.stringify(response));

  return { props: { musicstyleData: result } };
}

export default function MusicStyleEdit({ musicstyleData }) {
  return (
    <>
      <div className='flex flex-wrap'>
        <div className='w-full'>
          <CardEditMusicStyle musicstyleData={musicstyleData} />
        </div>
...
MusicStyleEdit.layout = BackOffice;
