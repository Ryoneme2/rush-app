...
function Account(userData) {
    ...
    async function changeInfo(changeData) {
        ...
        let date = new Date(changeData.date + "-" + changeData.month + "-" + changeData.year)
    ...
    async function changePassword(changePassData) {
        ...
        if (changePassData.oldPass == "" || changePassData.newPass == "" || changePassData.ConfirmPass == "") { return; }
        ...
        <div className="container mx-auto flex flex-col">
            <div className="mt-12 account-padding">
                ...
                <div className="flex flex-row space-x-12 font-bold mb-12">
                    <button className={menu == "info" ? activeBTN : null} onClick={() => { setMenu("info") }}>Personal infomation</button>
                    <button className={menu == "password" ? activeBTN : null} onClick={() => { setMenu("password") }}>Change password</button>
                </div>
            </div>
            {
                menu == "info" &&
                    ...
                    <div className="hidden lg:block border border-black rounded-md shadow-md p-6 xs:w-11/12 lg:w-5/12 xs:mx-auto mb-28 lg:mx-0 lg:mb-0">
                        ...
                        <form action="" onSubmit={handleSubmit(changeInfo)}>
                            <div className="flex flex-col mb-6">
                                ...
                                <input type="text" className="rounded-md" {...register("firstName")} defaultValue={userData.userData.FIRST_NAME ?? ""} required />
                            </div>
                            <div className="flex flex-col mb-6">
                                <div>
                                    <span>Last name</span><span className="text-red-500">*</span>
                                </div>
                                <input type="text" className="rounded-md mb-2" {...register("lastName")} defaultValue={userData.userData.LAST_NAME ?? ""} required />
                            ...
                            <div className="flex flex-col mb-6">
                                ...
                                <div className="flex flex-row w-full mb-2">
                                    <select className="flex-1  rounded-md" {...register("date")} value={Number(new Date(userData.userData.BIRTH_DATE).getDate())} required>{GetDate()}</select>
                                    <select className="flex-1 mx-1 rounded-md" {...register("month")} value={months[Number(new Date(userData.userData.BIRTH_DATE).getMonth())]} required>{GetMonths()}</select>
                                    <select className="flex-1  rounded-md" {...register("year")} value={Number(new Date(userData.userData.BIRTH_DATE).getFullYear())} required> {GetYear()}</select>
                                </div>
                            ...
                            <div className="flex flex-col mb-6">
                                ...
                                <input type="text" className="rounded-md" {...register("email")} defaultValue={userData.userData.ACCOUNT_INTERNAL[0].EMAIL ?? ""} required />
                            </div>
                            <div className="flex flex-col mb-12">
                                ...
                                <input type="text" className="rounded-md" {...register("phone")} defaultValue={userData.userData.PHONE ?? ""} />

                            </div>

                            <div className="flex flex-col mb-6">
                                <button type="submit" className="bg-primary font-bold text-white text-center p-2 rounded-md">Save</button>
                            </div>
                        </form>
                    ...
                    <div className="hidden lg:block border border-black rounded-md shadow-md p-8 xs:w-11/12 lg:w-5/12 xs:mx-auto lg:mx-0">
                        ...
                        <div className="flex flex-col mb-8">
                            <div>
                                ...
                                <span>In order to change your password you must enter your old password correctly and then following by new password and repeat the new password in Confirm password session</span>
                        ...
                        <form onSubmit={handleSubmit(changePassword)}>
                            <div className="flex flex-col space-y-6 mb-12">
                                <div className="w-full flex flex-row justify-between border-1 rounded-md pr-3">
                                    <input type={showOldPass ? "text" : "password"} minLength={6} className=" rounded-md w-11/12 border-0" placeholder="Old password" {...register("oldPass")} required />
                                    <i className="far fa-eye self-center text-lg cursor-pointer" onClick={() => { setShowOldPass(!showOldPass) }}></i>
                                </div>
                                <div className="w-full flex flex-row justify-between border-1 rounded-md pr-3">
                                    <input type={showNewPass ? "text" : "password"} minLength={6} className=" rounded-md w-11/12 border-0" placeholder="New password" {...register("newPass")} required />
                                    <i className="far fa-eye self-center text-lg cursor-pointer" onClick={() => { setShowNewPass(!showNewPass) }}></i>
                                </div>
                                <div className="w-full flex flex-row justify-between border-1 rounded-md pr-3">
                                    <input type={showNewConPass ? "text" : "password"} minLength={6} className=" rounded-md w-11/12 border-0" placeholder="Confirm password" {...register("ConfirmPass")} required />
                                    <i className="far fa-eye self-center text-lg cursor-pointer" onClick={() => { setShowNewConPass(!showNewConPass) }}></i>
                                </div>
                            </div>

                            <div className="flex flex-col mb-5">
                                <button className="bg-primary font-bold text-white text-center p-2 rounded-md" type="submit">Change password</button>
                            </div>
                        </form>
                    </div>
