  ...
  const response = await prisma.bOOKING.findMany({
    ...
    include: {
      ...
      BOOKING_TABLES: {
        select: {
          TABLE: {
            select: { ID: true, NAME: true, SEATS_AMOUNT: true, AREA: true },
          },
        },
      },
      BOOKING_PACKAGE_SELECT: { select: { BOOKING_PACKAGE: true } },
    },
...
export default function MyBooking({ data }) {
  ...
  area.map((a) => {
    dataArea.push(
      data.map((d) => {
        return {
          AREA: a,
          TABLE: d.BOOKING_TABLES.map((bt) => {
            if (area.includes(bt.TABLE.AREA.NAME) && a == bt.TABLE.AREA.NAME) {
              return {
                ID: bt.TABLE.ID,
                NAME: bt.TABLE.NAME,
                SEATS_AMOUNT: bt.TABLE.SEATS_AMOUNT,
              };
            }
          }),
        };
  ...
  const activeBTN =
    'text-primary underline underline-offset-4 underline-thickness';
        ...
        <div className='py-8 md:pl-8 md:pr-5 w-full md:w-6/12 lg:w-5/12 bg-white sm:rounded-lg'>
          <div className='md:w-full lg:w-8/12'>
            <h2 className='font-bold text-xl mb-4'>
              No booking have been made.
            </h2>
            <p className='mb-12'>
              It's time to explore a fun night that will be one you will
              remember for a long time!
            </p>
            <Link href='/'>
              <button className='bg-primary font-bold text-white w-full py-2 rounded-lg'>
                Explore
              </button>
            </Link>
          </div>
        ...
        if (
          ...
          return (
            <HistoryCard
              key={'Confirm-' + index + 1}
              IsHistory={false}
              bookingData={throwData[index]}
              ImagePath={i.RESTAURANT?.RESTAURANT_GALLERY[0]?.FILE_PATH}
              Name={i.RESTAURANT.NAME}
              bDate={new Date(i.BOOK_DATETIME)}
              Guests={i.GUESTS_AMOUNT}
              Location={i.RESTAURANT.ADDRESS}
              Status='Confirm'
              Firstname={!session?.fname ? '' : String(session.fname)}
              Lastname={!session?.lname ? '' : String(session.lname)}
              TableList={i.ZONETABLE}
              TableCount={i.BOOKING_TABLES.length}
              PackageList={i.BOOKING_PACKAGE_SELECT}
              Condition={i.RESTAURANT?.CONDITION}
              Fee={(totalPrice * parseFloat(i.RESTAURANT.FEE)) / 100}
              TotalPrice={
                totalPrice + (totalPrice * parseFloat(i.RESTAURANT.FEE)) / 100
              }
              LineID='@886jxuaj'
              LineLink='https://lin.ee/hWLrvSM'
            />
          );
        }
      });
      ...
      <div className='container mx-auto hidden lg:flex flex-col px-0 mb-20'>
        <div className='mt-5 mb-7 px-0'>
          ...
          <div className='flex flex-row space-x-6 font-bold'>
            <button
              className={menu == 'Confirm' ? activeBTN : null}
              onClick={() => {
                setMenu('Confirm');
              }}
            >
            ...
            <button
              className={menu == 'Pending' ? activeBTN : null}
              onClick={() => {
                setMenu('Pending');
              }}
            >
            ...
            <button
              className={menu == 'History' ? activeBTN : null}
              onClick={() => {
                setMenu('History');
              }}
            >
